function (beta = myNorm, pheno = myLoad$pd$Sample_Group, compare.group = NULL, 
  ...
  if (method == "Bumphunter") {
    ...
    DMR <- Bumps$table[which(Bumps$table$p.valueArea <= 
                               adjPvalDmr), ]
    ...
    myDmrGr$geneSymbol <- sapply(genes, function(x) paste(unique(unlist(strsplit(x$geneSymbol, 
                                                                                 ";"))), collapse = ";"))
    rm(genes)
    myDmrGr <- as(cbind(as.data.frame(myDmrGr), dmrGroupMeans), 
                  "GRanges")
    rm(dmrGroupMeans)
    interplot <- function(illumina450Gr, lassoSizes) {
      ...
      axis(2, at = c(0, max(0.3 * sqrt(lassoSizes))), 
           labels = F)
    }
    if (Rplot) 
    ...
    if (arraytype == "450K") {
      myannotation <- cpg.annotate(datatype = "array", 
                                   ...
                                   analysis.type = cateMethod, arraytype = "450K", what = "M") #annotation = c(array = "IlluminaHumanMethylation450k", annotation = "ilmn12.hg19")
    }
    else {
      myannotation <- cpg.annotate(datatype = "array", 
                                   fdr = fdr, myMs, design = design, coef = ncol(design), 
