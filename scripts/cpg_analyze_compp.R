...
print(paste0("Running with argument: ",""))
print(args)
...
which.cancer.type <- script_params(vector = types,
                                   submission = T, args = args, parallel = F)


meth <- T

print(which.cancer.type)
path.to.metadata <- paste0("metadata/results_dmp_new/",which.cancer.type) #dmp, deg does not have tissue subdirectory
files.list <- list.files(path.to.metadata, pattern=paste0(which.cancer.type,"_GDSC_meth_results"), all.files=FALSE, full.names=TRUE)
...
if(gex){
  ...
  for(files.list.index in 1:length(files.list)){
    ...
    if(files.list.index == 1){
      tmp <- test$DAT_gex$resp
      results.tmp <- test$gex_asso
    }else{ # make the response dataframe <------
      #if(length(row.names(test$DAT_gex$resp)[!row.names(test$DAT_gex$resp) %in% row.names(tmp)])>0){
      #  cl <- row.names(test$DAT_gex$resp)[!row.names(test$DAT_gex$resp) %in% row.names(tmp)]
      #  tmp <- rbind(tmp,rep(NA, nrow(tmp)))
      #  row.names(tmp)[(nrow(tmp)-length(cl)+1):(nrow(tmp))] <- cl
      #  row.names(tmp)
      #}
      tmp <- cbind(tmp[intersect(row.names(tmp),row.names(test$DAT_gex$resp)),],test$DAT_gex$resp[intersect(row.names(tmp),row.names(test$DAT_gex$resp)),])
      results.tmp <- rbind(results.tmp, test$gex_asso)
    }
    drugs.list <- unique(test$gex_asso$indep)
  ...
  test$DAT_gex$resp <- tmp
  test$gex_asso <- results.tmp
  results.gex <- test
  #save(response, file = paste0(path.to.metadata,"/",which.cancer.type,".response.RData"))
  save(results.gex, file = paste0(path.to.metadata,"/",which.cancer.type,".genes.RData"))
}
